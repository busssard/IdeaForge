...
/// Social sharing buttons for ideas.
/// Generates share links for Twitter/X, LinkedIn, Reddit, Facebook, and Copy Link.
#[component]
pub fn ShareButtons(
    /// The full URL of the idea to share.
    url: String,
    /// The idea title for share text.
    title: String,
    /// Short summary for share text.
    ...
    let copy_link = move |_: web_sys::MouseEvent| {
        let url = url_for_copy.get_value();
    ...
    view! {
        <div class="share-buttons">
            ...
            <button
                ...
                on:click=copy_link
                title=move || if copied.get() { "Copied!" } else { "Copy link" }
            >
                {move || if copied.get() {
                    view! {
                        <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
